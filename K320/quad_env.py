...
class RewardWeights:
    ...
    forward_vel:    float =  0.5    # reward forward velocity (vx)
    lateral_pen:    float = -0.2    # penalise sideways drift (vy)
    roll_pen:       float = -0.5    # penalise |roll|
    pitch_pen:      float = -0.5    # penalise |pitch|
    height_pen:     float = -1.0    # penalise height deviation
    ...
    height_tol:     float = 0.06    # +/- metres before penalty (slightly more tolerant)
